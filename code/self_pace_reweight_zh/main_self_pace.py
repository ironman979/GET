...
def train(model, train_dataset, train_loader, device, cfg, tokenizer):
    ...
    for epoch in range(cfg.epochs):
        ...
        for batch_idx, (index,sample) in enumerate(train_loader):
            ...
            loss = criterion(epoch, index, weight, outputs_1,outputs_2,labels=labels,device=device,tokenizer=tokenizer)
            loss.backward()
...
def main():
    ...
    model = MT5ForConditionalGeneration.from_pretrained(cfg.pretrained_model_path)
    ...
    if cfg.train:
        ...
        #model.load_state_dict(torch.load(cfg.epoch2))
