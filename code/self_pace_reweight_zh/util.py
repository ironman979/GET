...
class SPLLoss(nn.NLLLoss):
    def __init__(self, *args, n_samples=0, **kwargs):
        ...
        self.threshold = 0.5
        #self.threshold_init = 5
        self.growing_factor = 2
        self.v = (torch.rand(n_samples) > 0.5).int().cuda()

    def forward(self, epoch, index, weight, outputs_1, outputs_2, labels, device, tokenizer):
        super_loss = calculate_loss_and_accuracy_stable(weight,outputs_1, outputs_2, labels, device, tokenizer)
...
def calculate_loss_and_accuracy_stable(weight,outputs_1, outputs_2, labels, device, tokenizer):
    ...
    for i in range(batch_size):
        ...
        kl_loss[i] =  (4 * kl_loss[i] + 0.5 * (loss_1+loss_2)) * weight[i]
...
def read_data_train(data_dir, tokenizer):
    ...
    for i in tqdm(range(len_all)):
        ...
        weight_ls[i] = torch.from_numpy(np.array(len(labels[1:-2])))
    reweight_ls = torch.nn.functional.softmax(weight_ls, dim = 0)
    '''
    for i in tqdm(range(len_all)):
        dic = json_data[i]
        ...
        labels = tokenizer.encode(label_text)
        #weight = reweight_ls[i]
        weight = torch.from_numpy(np.array(len(labels[1:-2])))/2
        ...
        final_result.append([input_ids, labels, input_text,weight])
    return final_result
